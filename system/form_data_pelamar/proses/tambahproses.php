...
if (isset($_SESSION['email'])) {
	if (isset($_SESSION['role'])) {
		echo '<script language="javascript">alert("Anda Sudah Login :)"); document.location="../../tampilan/index.php";</script>';
	}
}
...
if ($_POST['submit']) {
	...
	if (
		in_array($ekstensi_1, $ekstensi_diperbolehkan) && in_array($ekstensi_2, $ekstensi_diperbolehkan) &&
		in_array($ekstensi_3, $ekstensi_diperbolehkan) && in_array($ekstensi_4, $ekstensi_diperbolehkan)
	) {
		if ($ukuran_1 < 1044070 || $ukuran_2 < 1044070 || $ukuran_3 < 1044070 || $ukuran_4 < 1044070) {
			...
			$no_telp = $_POST['no_telp']; //foto
			$ft_ktp = $ft_ktp_1;
			$skck = $skck_2;
			$ijazah = $ijazah_3;
			$ft_transkip = $ft_transkip_4;
			if (
				$nama_lengkap == null and
				$alamat == null and $tanggal_lahir == null and $jenis_kelamin == null and $tamat_sekolah == null and $jurusan == null and
				$no_telp = null and $ft_ktp = null and $skck = null and $ijazah = null and $ft_transkip = null
			) {
				echo "Data Form Harus Disi Semua!!!";
			} //foto
			$data = mysqli_query(
				$koneksi,
				"INSERT INTO tb_pelamar VALUES('','$nama_lengkap', '$alamat', '$tmpt_lahir', '$tanggal_lahir', '$jenis_kelamin', '$tamat_sekolah', '$jurusan', '$ft_ktp', '$skck', '$ijazah', '$ft_transkip', '$no_telp')"
